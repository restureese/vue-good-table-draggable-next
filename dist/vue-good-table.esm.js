/*!
  * vue-good-table-draggable-next v0.1.1
  * (c) 2021-present Boris Flesch <boris@singlequote.net>
  * (c) 2017-2021 xaksis <shay@crayonbits.com>
  * @license MIT
  */
import require$$0, { openBlock, createElementBlock, toDisplayString, withModifiers, createElementVNode, withKeys, resolveComponent, withDirectives, Fragment, renderList, createCommentVNode, vModelSelect, createVNode, normalizeClass, renderSlot, withCtx, normalizeStyle, createTextVNode, createSlots, createBlock } from 'vue';
      ...
      if (!dragStarted) {
        ...
        if (!~multiDragElements.indexOf(dragEl$1)) {
          ...
          if (evt.shiftKey && lastMultiDragSelect && sortable.el.contains(lastMultiDragSelect)) {
            var lastIndex = index$2(lastMultiDragSelect),
                currentIndex = index$2(dragEl$1);
      ...
      if (dragStarted && this.isMultiDrag) {
        ...
        if ((parentEl[expando].options.sort || parentEl !== rootEl) && multiDragElements.length > 1) {
          var dragRect = getRect(dragEl$1),
              multiDragIndex = index$2(dragEl$1, ':not(.' + this.options.selectedClass + ')');
    ...
    eventProperties: function eventProperties() {
      ...
      multiDragElements.forEach(function (multiDragElement) {
        ...
        if (folding && multiDragElement !== dragEl$1) {
          ...
          newIndex = index$2(multiDragElement, ':not(.' + _this3.options.selectedClass + ')');
        } else {
          newIndex = index$2(multiDragElement);
        }
      ...
      item: withCtx(({ element, index }) => [
        (!element.hidden)
          ? (openBlock(), createElementBlock("th", {
              ...
              class: normalizeClass($options.getHeaderClasses(element, index)),
              style: normalizeStyle($data.columnStyles[index]),
              "aria-sort": $options.getColumnSortLong(element),
              "aria-controls": `col-${index}`
            }, [
              renderSlot(_ctx.$slots, "table-column", { column: element }, () => [
                createTextVNode(toDisplayString(element.label), 1 /* TEXT */)
              ]),
              ($options.isSortableColumn(element))
                ? (openBlock(), createElementBlock("button", {
                    key: 0,
                    onClick: $event => ($options.sort($event, element))
                  }, [
                    createElementVNode("span", _hoisted_7$2, " Sort table by " + toDisplayString(element.label) + " in " + toDisplayString($options.getColumnSortLong(element)) + " order ", 1 /* TEXT */)
